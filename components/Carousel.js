...
export default function Home({carouselProperties}) {
  ...
  <div className="carousel-inner relative w-full h-[180px] md:h-[360px] xl:h-[650px] rounded-[20px] overflow-hidden">

    {pictures.map((ele, index) => {
      
      return(
        <div key={index} className={`carousel-item ${index == currentImage ? "active z-20" : "z-10"} block relative float-left w-full`}>
      <Image layout='responsive' width={12} height={8}
        src={`/images/${ele.url}`}
